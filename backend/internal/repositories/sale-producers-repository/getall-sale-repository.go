...
func (r *saleRepository) GetAll(ctx context.Context) (*[]entities.SaleResponse, *customerror.CustomError) {
	...
	q := `SELECT sales_producers.value, sales_producers.commission, sales_producers.date, producers.name as "producer", products.name as "product" FROM sales_producers INNER JOIN producers ON sales_producers.producer_id = producers.id INNER JOIN products ON sales_producers.product_id = products.id`
